...
for filename in sorted(glob.glob('../dataset/*.csv')):
    ...
    if (productivity_performance_model.params[1] == 0): # if regression coefficient = 0, skip dataset
        ...
        if filenum == 1:
            ...
            with open('../results/prod_activityopen.txt', 'a') as fh:
                fh.write(filename)
                fh.write("\n")
                fh.write(productivity_performance_model.summary().as_text())
                fh.write("\n")
    ...
    pval_pr_opened = productivity_performance_model.pvalues.loc['pr_opened']
...
dfi.export(df, '../results/pr_opened_summary.png')
...
dfi.export(df2_transposed, '../results/pr_opened_avgs.png')