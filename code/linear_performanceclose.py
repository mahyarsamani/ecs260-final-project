...
for filename in sorted(glob.glob('../dataset/*.csv')):
    ...
    if (productivity_performance_model.params[1] == 0): # if regression coefficient = 0, skip dataset
        ...
        if filenum == 1:
            with open('../results/prod_perfclose.txt', 'w') as fh:
                fh.write(filename)
                fh.write("\n")
                fh.write(productivity_performance_model.summary().as_text())
                fh.write("\n")
        else: 
            with open('../results/prod_perfclose.txt', 'a') as fh:
                fh.write(filename)
                fh.write("\n")
                fh.write(productivity_performance_model.summary().as_text())
                fh.write("\n")
    ...
    pval_issue_closed = productivity_performance_model.pvalues.loc['issue_closed']
...
dfi.export(df, '../results/issue_closed_summary.png')
...
dfi.export(df2_transposed, '../results/issue_closed_avgs.png')